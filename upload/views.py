    ...
    if request.method == 'POST':
        ...
        if not files:
            ...
            try:
                for file in files:
                    suffix = os.path.splitext(file.name)[1]
                    fname = "diary_%s_%06d%s"%(time.strftime('%Y%m%d%H%M%S',time.localtime()), random.randint(0,100000), suffix)
                    path = dirs + fname
                    f = open(path,'wb')
                    for line in file.chunks():
                        f.write(line)
    ...
    if request.method == 'POST':
        ...
        if not files:
            ...
            try:
                for file in files:
                    suffix = os.path.splitext(file.name)[1]
                    fname = "dinamic_%s_%06d%s"%(time.strftime('%Y%m%d%H%M%S',time.localtime()), random.randint(0,100000), suffix)
                    path = dirs + fname
                    f = open(path,'wb')
                    for line in file.chunks():
                        f.write(line)
